...
def generar(d: Path, seed: int = 7, n_tm=2000, n_tu=3000, n_am=1500, n_mr=1200, n_cdc=600) -> dict:
    ...
    # ---- catálogos
    (d / "tm_estaciones.csv").write_text(
        "estacion_id,nombre,linea,zona,lat,lon\n"
        + "".join(f"TM-L1-{i:02d},Estación Centra Sur - Centro {i},L1,Zona {i + 3},14.4{i},-90.5{i}\n" for i in range(1, 7)))
    (d / "tu_paradas.csv").write_text(
        "cod_parada,descripcion,ruta,sector\n"
        + "".join(f"R-101-{i},Parada {i} ruta R-101,R-101,Z{i + 3}\n" for i in range(1, 6)))
    (d / "mr_estaciones.csv").write_text(
        "id_estacion,nombre_estacion,zona_nombre,km\n"
        + "".join(f"{i},MR {i:02d},Zona 12,{i * 0.95:.2f}\n" for i in range(1, 23)))
    (d / "am_estaciones.csv").write_text(
        "station_code,station_name,axis,district\n"
        + "".join(f"AM1{i},Aerometro Eje 1 - Torre {i},Eje 1,Zona 12\n" for i in range(1, 8)))
    ...
    for i in range(1, n_mr + 1):
        ...
        if i % 25 == 0:
            lineas.append(f'{{"trip_id": {i}, "card": "MR{rnd.randint(1, 900):07d}", '
                          f'"entry": {{"station": 12, "ts": "2026-06-0{rnd.randint(1, 5)}T19:11:11"}}, '
                          f'"exit": null, "fare_gtq": {fare}, "duration_s": null{extra}}}')
        else:
            lineas.append(f'{{"trip_id": {i}, "card": "MR{rnd.randint(1, 900):07d}", '
                          f'"entry": {{"station": 12, "ts": "2026-06-0{rnd.randint(1, 5)}T19:11:11"}}, '
                          f'"exit": {{"station": 15, "ts": "2026-06-01T19:23:31"}}, "fare_gtq": {fare}, "duration_s": 740{extra}}}')
    ...
    return {
        ...
        "tm_estaciones": {"filas": 6, "malformadas": 0}, "tu_paradas": {"filas": 5, "malformadas": 0},
